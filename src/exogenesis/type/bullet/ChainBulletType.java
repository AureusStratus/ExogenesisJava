...
public class ChainBulletType extends ExoBulletType{
	...
	public void init(Bullet b){
		...
		if(!quietShoot || !points.isEmpty()){
			ExoFunc.shuffle(points, ExoFunc.rand(b.owner.id()));
	...
	public void draw(Bullet b){
		...
		for(int i = 1; i < data.size; i++){
			...
			Drawf.light(from.getX(), from.getY(), to.getX(), to.getY(), Lines.getStroke() * 3f + 0.5f, hitColor, b.fout());
		}
	}
