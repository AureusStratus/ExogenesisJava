...
import exogenesis.util.func.ExoInterp;
...
//        Lines.linePoint(x2, y2);
    ...
    public static Color markColor(Team target){
        return target == Vars.player.team() ? ExoPal.ally : ExoPal.hostile;
    }
    
    public static Color markColor(Teamc target){
        return target.team() == Vars.player.team() ? ExoPal.ally : ExoPal.hostile;
    }
    ...
    public static float rotator_90(float in /*(0, 1)*/, float margin){
        return 90 * ExoInterp.pow10.apply(Mathf.curve(in, margin, 1 - margin));
    }
    
    public static float rotator_120(float in /*(0, 1)*/, float margin){
        return 120 * ExoInterp.pow10.apply(Mathf.curve(in, margin, 1 - margin));
    }
