package exogenesis.util.feature;
...
public class PositionLightning {
	...
	public static final Effect posLightning = (new Effect(lifetime, 1200.0f, e -> {
		if(!(e.data instanceof exogenesis.util.struct.Vec2Seq)) return;
		exogenesis.util.struct.Vec2Seq lines = e.data();
	...
	public static void createBoltEffect(Color color, float width, exogenesis.util.struct.Vec2Seq vets) {
		if(cornerSpark){
			vets.each(((x, y) -> {
				if(Mathf.chance(0.0855)) ExoFx.colorSparkShoot.at(x, y, rand.random(2f + width, 4f + width), color);
			}));
		}
		posLightning.at((vets.firstTmp().x + vets.peekTmp().x) / 2f, (vets.firstTmp().y + vets.peekTmp().y) / 2f, width, color, vets);
	}
	
	private static exogenesis.util.struct.Vec2Seq computeVectors(FloatSeq randomVec, Position from, Position to){
		int param = randomVec.size;
		float angle = from.angleTo(to);

		exogenesis.util.struct.Vec2Seq lines = new exogenesis.util.struct.Vec2Seq(param);
