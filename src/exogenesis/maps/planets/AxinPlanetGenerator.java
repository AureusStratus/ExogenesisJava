...
public class AxinPlanetGenerator extends PlanetGenerator{
    ...
    divine1 = Color.valueOf("#c32121"), divine2 = Color.valueOf("#e87694"),
            ...
            axinstone1 = Color.valueOf("#303542"), axinestone2 = Color.valueOf("#14171d"),
            lightningStone1 = Color.valueOf("#3a4360"), lightningStone2 = Color.valueOf("#1e2538"),
            dacite1 = Color.valueOf("#ece8d6"), dacite2 = Color.valueOf("#a08c7a"),

    Iced1 = Color.valueOf("#b6b8d4"), Iced2 = Color.valueOf("#e2f9ff"),

    oceanColor1 = Color.valueOf("#eff0fb"), oceanColor2 = Color.valueOf("#b5bcf7"),
            ...
            shoreline1 = Color.valueOf("#1b2249"), shoreline2 = Color.valueOf("#0b1021"),
            IcedBeach1 = Color.valueOf("#474952"), IcedBeach2 = Color.valueOf("#1e2026"),

    desert1 = Color.valueOf("#8884b7"), desert2 = Color.valueOf("#57547d");
    ...
    public void getColor(Vec3 position, Color out) {
        ...
        Vec3 pos = new Vec3(position).scl(6.5f);

        float height = rawHeight(position);
        float depth = Simplex.noise3d(baseSeed, 6, 0.7f, 1f, position.x, position.y, position.z);
        ...
        if ((desertBiome * depth )> 0.32 && height < 0.56f && height > waterLevel + 0.1f && Math.abs(position.y) < 0.37){
            ...
            out.set(shoreline1).write(out).lerp(shoreline2, height * 1.5f);
            return;
        }
        ...
        if(height > waterLevel){
            ...
            if(Math.abs(position.y) + height > 1.2f){
                ...
                if (pole + height > 0.3) {
                    out.set(shoreline1).lerp(shoreline2, Mathf.clamp(Mathf.round(height, 0.15f))).a(0.8f);
                    return;
            ...
            out.set(axinstone1).lerp(axinestone2, Mathf.clamp(Mathf.round(depth, 0.15f))).a(0.5f);
            return;
        }
        // Oceans
    ...
    float rawHeight(Vec3 position){
        ...
        float mountains = Ridged.noise3d(baseSeed, pos.x, pos.y, -3, 4, 0.5f);
        float glaciers = Ridged.noise3d(baseSeed+1, position.x, position.y, position.z, 7, 0.5f);
        float base = (Mathf.pow(Simplex.noise3d(baseSeed, 3, 0.55f, 0.2, pos.x, pos.y, pos.z), 2.3f) + waterOffset) / (1f + waterOffset);
    ...
    public void addWeather(Sector sector, Rules rules){
        ...
        for(Weather.WeatherEntry w : rules.weather){
